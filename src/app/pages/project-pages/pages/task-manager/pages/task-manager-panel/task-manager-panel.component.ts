...
export class TaskManagerPanelComponent implements  OnDestroy,OnInit{
  ...
  ngOnInit(){
    this.ar.parent.paramMap.subscribe((params)=>{
      const tab = this.ar.snapshot.paramMap.get('tab');
  ...
  tabNav(index:number){
    ...
    this.router.navigate([tab],{relativeTo:this.ar.parent}).then(()=>{
      this.setTabIndex(tab);
    })
  }
  ngOnDestroy() {
    this.taskSelected ? this.taskService.userOutTask(this.taskSelected._id) : null;
