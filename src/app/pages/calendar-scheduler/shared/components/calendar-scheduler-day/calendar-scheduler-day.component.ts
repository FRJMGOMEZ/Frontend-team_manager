...
export class CalendarSchedulerDayComponent {
  ...
  taskRender(taskDiv: ElementRef, task:TaskModel, hour: Hour, size: 'small' | 'regular') {
  ...
  hasTask( task: TaskModel, hour: Hour){
  ...
  renderTask(taskDiv:ElementRef,size: 'small' | 'regular', percent:number,task:TaskModel){
    if (size === 'regular') {
