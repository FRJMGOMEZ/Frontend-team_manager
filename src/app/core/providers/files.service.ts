...
export class FilesService {
  ...
  deleteFile(file:FileModel){
    ...
    const request = this.http.delete(url).pipe(
      ...
      tap((file: FileModel) => this.spreadFile(file,'DELETE')));
  ...
  downloadFile(data: { src: any, file: FileModel }): Observable<any> {
    const { src, file } = data;
    return this.lpDialogsService.openConfirmDialog('DOWNLOAD FILE?', file.title).pipe(
      switchMap((res: boolean) => {
      if (res) {
        const request: Observable<any> = !src.includes('blob') ? this.getDbFile(file.name).pipe(map((blob: Blob) => {  return { src: URL.createObjectURL(blob), title: file.title } })) : of({ src, title: file.title }).pipe(tap(() => this.loadSpinnerService.state.next(false)));
        return request.pipe(
          tap((res: { src: any, title: string }) => {
