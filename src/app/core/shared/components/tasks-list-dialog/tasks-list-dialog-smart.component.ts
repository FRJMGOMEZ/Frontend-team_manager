...
export class TasksListDialogSmartComponent implements OnInit {
    ...
    getTasks(date){
        ...
        this.taskService.getTasks('day', OOService.toQueryString({ project: this.selectedProject, from: timeRange[0], to: timeRange[1] })).pipe(map((res: any) => { return res.tasks })).subscribe((tasks:TaskModel[])=>{
            this.tasks = tasks; 
        })
    }
