...
export class TaskService {
  ...
  getTasksByTimeRange(selector: string, range: number[], projectId: string = '', recursiveFilter?: number) {
    let url = `${URL_SERVICES}tasks-by-time-range/${selector}?from=${range[0]}&to=${range[1]}&projectId=${projectId}&recursiveFilter=${recursiveFilter}`
  ...
  editionBanned(task: TaskModel) {
    ...
    if (DateOperationsService.dateComparison(new Date(today.getFullYear(), today.getMonth(), today.getDate(), 0, 0, 0, 0), new Date(task.startDate))) {
      let taskStartDateTime;
      const startTime = task.startTime ? task.startTime.split(':').reduce((acum, time, index) => { return index === 0 ? acum + Number(time) : (acum + (Number(time) / 60)) }, 0) * 3600000 : 0;
      taskStartDateTime = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 0, 0, 0, 1).getTime() + startTime;
      if (new Date().getTime() > taskStartDateTime) {
        task.editable = false;
        return true
      }
      task.editable = true;
